...
export default function DocumentUploadCard({ document, status, onUpdate }) {
  ...
  const [summaries, setSummaries] = useState([]) // [{ filename|url, resumo }]
  ...
  const pushUpdate = useCallback(
    (extra = {}) => {
      onUpdate?.({
        // agora "arquivo_urls" vira só uma lista de nomes (não usamos mais URLs)
        arquivo_urls: selectedFiles.map(f => f.name),
        ...extra
      })
    },
  ...
  /** Envia imediatamente os arquivos selecionados para o n8n */
  const sendFilesToN8n = useCallback(
    async files => {
      ...
      try {
        const data = await uploadToN8N(files, { documentKey: document?.key })
        // normaliza resposta em array
        const arr = Array.isArray(data) ? data : [data]
        setSummaries(arr)
      } catch (e) {
        console.error(e)
        setErrorMsg(e?.message || 'Falha ao enviar arquivos ao n8n.')
      } finally {
  ...
  /** Seleção do input/drag&drop (aceita vários) */
  const handleSelectedFiles = useCallback(
    async fileList => {
      ...
      // etiqueta amigável para o pai
      const label = `Arquivos: ${files.map(toDisplayName).join(', ')}`
      pushUpdate({ [textFieldName]: label })

      // dispara upload imediato pro n8n
      setIsUploading(true)
      try {
        await sendFilesToN8n(files)
      } finally {
        setIsUploading(false)
      }
    },
    [pushUpdate, textFieldName, sendFilesToN8n]
  )
  ...
  const handleTextSubmit = () => {
    ...
    onUpdate?.({ [textFieldName]: textContent.trim() })
  }

  const removeFile = indexToRemove => {
    const nf = selectedFiles.filter((_, i) => i !== indexToRemove)
    setSelectedFiles(nf)
    if (nf.length) {
      const label = `Arquivos: ${nf.map(toDisplayName).join(', ')}`
      onUpdate?.({ arquivo_urls: nf.map(f => f.name), [textFieldName]: label })
    } else {
      onUpdate?.({ status: 'pendente', arquivo_urls: [] })
      setSummaries([])
      setErrorMsg('')
    }
  }
                  ...
                  {isUploading || isSummarizing ? (
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className="text-gray-600">
                        {isUploading ? 'Enviando...' : 'Processando no n8n...'}
                      </p>
                    </div>
                  ) : (
                    <div className="flex flex-col items-center gap-2">
